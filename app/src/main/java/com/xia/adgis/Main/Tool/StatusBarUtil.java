...
public class StatusBarUtil {
    ...
    //<editor-fold desc="沉侵">
    public static void immersive(Activity activity, boolean isDark) {
        if(isDark){
            immersiveDark(activity.getWindow(), DEFAULT_COLOR, DEFAULT_ALPHA);
        }else {
    ...
    public static void immersiveDark(Window window, int color, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
        window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
        window.setStatusBarColor(mixtureColor(color, alpha));
