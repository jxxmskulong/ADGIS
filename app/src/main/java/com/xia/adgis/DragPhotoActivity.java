...
import com.xia.adgis.Utils.DragPhotoView.DragPhotoView;
...
public class DragPhotoActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mViewPager = (ViewPager) findViewById(R.id.drag_photo_viewpager);
        //进入的索引
        index = getIntent().getIntExtra("index",0);

        //所进入需要加载的图片路径
        mList = (ArrayList<String>)getIntent().getSerializableExtra("path");
        ...
        for (int i = 0; i < mPhotoViews.length; i++) {
            mPhotoViews[i] = (DragPhotoView) View.inflate(this, R.layout.item_drag_photo, null);
            Glide.with(DragPhotoActivity.this).load(mList.get(i)).
